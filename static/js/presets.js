  ...
  if (delBtn) {
    delBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        if (isBuiltin) {
          const hidden = JSON.parse(localStorage.getItem('radian-hidden-presets') || '[]');
          if (!hidden.includes(charName)) hidden.push(charName);
          localStorage.setItem('radian-hidden-presets', JSON.stringify(hidden));
        }
...
function _populateCharSelect() {
  ...
  const hiddenPresets = JSON.parse(localStorage.getItem('radian-hidden-presets') || '[]');
...
function initPersistentChat() {
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      const charSessions = JSON.parse(localStorage.getItem('radian-char-sessions') || '{}');
      charSessions[sessionId] = charName;
      localStorage.setItem('radian-char-sessions', JSON.stringify(charSessions));
