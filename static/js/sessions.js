...
let _sortMode = Storage.get('radian-session-sort') || 'active'; // default to last active
...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'Radian Chat';
...
const FOLDER_STATE_KEY = 'radian-folder-state';
const FOLDER_ORDER_KEY = 'radian-folder-order';
...
export async function selectSession(id, { keepSidebar = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to Radian kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'Radian Chat';
    }
    ...
    if (window.documentModule) {
      ...
      const meta = sessions.find(s => s.id === id);
      const shouldOpen = localStorage.getItem('radian-doc-open-' + id) === '1';
...
export function setSortMode(mode) {
  _sortMode = mode || null;
  if (mode) Storage.set('radian-session-sort', mode);
  else Storage.remove('radian-session-sort');
  renderSessionList();
}
