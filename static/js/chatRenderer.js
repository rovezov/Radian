...
const _ACTION_RECENTS_KEY = 'radian-msg-actions-recent';
 ...
 */
const _USER_ACTION_RECENTS_KEY = 'radian-user-actions-recent';
    ...
    ctxRing.addEventListener('click', (e) => {
      ...
      if (compactBtn) {
        compactBtn.addEventListener('click', async (e) => {
          ...
          compactRole.textContent = 'Radian';
...
export function addMessage(role, content, modelName, metadata) {
  try {
    ...
    var _roleText = role === 'user' ? 'You' : (isSlash || isCompacted) ? 'Radian' : shortModel(resolvedModel);
