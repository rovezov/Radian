...
function _isPlanJson(text) {
  try {
    ...
    return !!(parsed && parsed.blueprint_type && parsed.objective && Array.isArray(parsed.steps));
  } catch (_) {
    return false;
  }
}
...
function _renderCard(plan) {
  ...
  const steps = (plan.steps || []).slice(0, 8)
    .map((s, i) => {
      ...
      <li class="orchestrator-plan-step">
        <div class="orchestrator-plan-step-title">${_esc(`${i + 1}. ${s.title || 'Step'}`)}</div>
  ...
  card.innerHTML = `
    ...
    <div class="orchestrator-plan-label">Planned steps</div>
...
export function enhanceOrchestratorPlanCards(root) {
  ...
  for (const block of blocks) {
    ...
    if (!plan || !Array.isArray(plan.steps) || !plan.blueprint_type) continue;
