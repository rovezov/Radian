...
const GROUP_STATE_KEY = 'radian-group-state';
...
export async function startGroup(models, parentSessionId) {
  ...
  try {
    ...
    try {
      const gids = JSON.parse(localStorage.getItem('radian-group-sessions') || '[]');
      if (!gids.includes(_parentSessionId)) { gids.push(_parentSessionId); localStorage.setItem('radian-group-sessions', JSON.stringify(gids)); }
    } catch (e) {}
  } catch (e) {
