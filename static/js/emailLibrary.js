...
function _publishActiveAccount() {
  try { window.__radianActiveEmailAccount = state._libAccountId || null; } catch (_) {}
}
...
export function openEmailLibrary(opts = {}) {
        ...
        <div class="admin-card" style="flex:1;flex-direction:column;display:flex;overflow:hidden;">
          ...
          <div class="memory-toolbar">
            <div class="memory-category-filters">
              ...
              <button class="memory-toolbar-btn email-reminders-clear-btn hidden" id="email-reminders-clear-btn" title="Permanently delete Radian reminder emails">
            ...
            <div class="email-search-wrap" style="position:relative;flex:1;min-width:140px;">
              ...
              <button class="memory-toolbar-btn email-reminder-toggle-inline hidden" id="email-reminder-btn" title="Show Radian reminder emails">
  ...
  document.getElementById('email-reminders-clear-btn')?.addEventListener('click', async () => {
    const ok = await styledConfirm('Permanently delete all Radian reminder emails?', {
      confirmText: 'Delete',
      cancelText: 'Cancel',
      danger: true,
    });
    if (!ok) return;
    try {
      const res = await fetch(`${API_BASE}/api/email/radian/reminders?permanent=1${_acct()}`, {
        method: 'DELETE',
...
const _BUBBLES_DISABLED_KEY = 'radian.email.bubblesDisabled';
...
const _SUMMARY_COLLAPSED_KEY = 'radian.email.summaryCollapsed';
