...
def _apply_radian_headers(msg, kind: str | None = None, ref_id: str | None = None):
    msg["X-Radian-Origin"] = RADIAN_MAIL_ORIGIN
    if kind:
        msg["X-Radian-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", kind)[:64]
    if ref_id:
        msg["X-Radian-Ref"] = re.sub(r"[^A-Za-z0-9_.:-]", "-", ref_id)[:128]
...
def setup_email_routes():
    ...
    def _list_emails_sync(folder, limit, offset, filter_, account_id, from_addr=None, has_attachments_only=False, owner=""):
        ...
        try:
            ...
            if filter_ == "unread":
                ...
                # Prefer the Radian marker header, but include the subject
                # fallback too. The fallback uses a distinct Radian prefix
                # so ordinary emails containing "Reminder" don't get mixed in.
                status, data = conn.search(
                    None,
                    f'(OR HEADER X-Radian-Kind "reminder" SUBJECT "Reminder (Radian):"{from_clause})',
                )
    ...
    @router.delete("/radian/reminders")
    async def delete_radian_reminder_emails(
        account_id: str | None = Query(None),
        permanent: bool = Query(False),
        owner: str = Depends(require_owner),
    ):
        """Delete email messages stamped as Radian reminders."""
        if account_id:
        ...
        try:
            ...
            with _imap(account_id, owner=owner) as conn:
                ...
                for folder_name in candidates:
                    ...
                    try:
                        ...
                        uids.update(_search_uids(conn, f'(HEADER X-Radian-Kind {_search_quote("reminder")})'))
                        uids.update(_search_uids(conn, f'(SUBJECT {_search_quote("Reminder (Radian):")})'))
                        for addr in own_addrs:
                            addr_q = _search_quote(addr)
                            uids.update(_search_uids(conn, f'(FROM {addr_q} SUBJECT {_search_quote("Reminder (Radian):")})'))
                            # Legacy reminders created before the Radian
            ...
            logger.error(f"delete_radian_reminder_emails failed: {e}")
            return {"success": False, "error": "Mail operation failed"}
    ...
    async def _send_email_sync(
        ...
        account_id=None, owner="", radian_kind=None, radian_ref=None,
    ):
        ...
        _apply_radian_headers(outer, radian_kind or "scheduled", radian_ref)
        if in_reply_to:
    ...
    async def schedule_email(req: dict, owner: str = Depends(require_owner)):
        ...
        try:
            ...
            conn.execute("""
                ...
                (id, to_addr, cc, bcc, subject, body, in_reply_to, references_hdr, attachments, send_at, created_at, status, account_id, radian_kind)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?)
            """, (
                sid,
                req.get("to", ""),
                ...
                req.get("radian_kind") or "scheduled",
            ))
            conn.commit()
            conn.close()
    ...
    async def send_email(req: SendEmailRequest, background_tasks: BackgroundTasks, owner: str = Depends(require_owner)):
        ...
        outer["Date"] = datetime.utcnow().strftime("%a, %d %b %Y %H:%M:%S +0000")
        outer["Message-ID"] = email.utils.make_msgid(domain="radian.local")
        ...
        if req.radian_kind:
            _apply_radian_headers(outer, req.radian_kind)
