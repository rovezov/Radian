...
# Override at deploy time via RADIAN_MAIL_ATTACHMENTS_DIR. Defaults to a
# subdir of the install's data/ tree so the app works out-of-the-box without
# a hardcoded /home/<user>/ path.
ATTACHMENTS_DIR = Path(os.environ.get("RADIAN_MAIL_ATTACHMENTS_DIR", str(DATA_DIR / "mail-attachments")))
...
def _init_scheduled_db():
    ...
    try:
        ...
        if "radian_kind" not in cols:
            conn.execute("ALTER TABLE scheduled_emails ADD COLUMN radian_kind TEXT")
    except Exception:
        pass
...
class SendEmailRequest(BaseModel):
    ...
    # Internal marker for Radian-generated mail (e.g. reminder, scheduled).
    radian_kind: Optional[str] = None
