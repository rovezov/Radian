...
Radian is a self-hosted AI workspace with privileged local capabilities. Please do not run it as a public, unauthenticated service.
...
- Rotate API keys, webhook secrets, and Radian API tokens if they appear in logs, screenshots, demos, or shared chats.
...
```bash
git status --short
git check-ignore -v .env data/auth.json data/app.db logs/compound.log radian.db
